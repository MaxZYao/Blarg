...
// Zaber addressing — CURRENT HARDWARE (2026-07-17): TWO RDQ150-AE57T10A
// stages, both on the one X-MCC (device 1, axes 1 and 2).
// CONVENTION: the mechanically-OUTER stage plugs into X-MCC axis port 1.
// If your cabling is the other way around, just swap the two AX values.
static const uint8_t OUTER_DEV = 1, OUTER_AX = 1;   // RDQ150-AE57T10A
static const uint8_t INNER_DEV = 1, INNER_AX = 2;   // RDQ150-AE57T10A
...
// The protocol speaks +/-180 deg, but a homed Zaber rotary addresses 0..360 —
// negative absolute targets would be rejected. Map both directions.
static double wrapTo360(double deg) {
...
static double zaberGetPosDeg(uint8_t dev, uint8_t axis, double stepsPerDeg) {
  ...
  return wrapTo180((double)r.getDataInt() / stepsPerDeg);
}

static void zaberMoveDeg(uint8_t dev, uint8_t axis, double deg, double stepsPerDeg) {
  long steps = (long)(wrapTo360(deg) * stepsPerDeg);
  connection.genericCommand(Command("move abs ", steps), dev, axis);
}

static void zaberHome(uint8_t dev, uint8_t axis) {
  connection.genericCommand("home", dev, axis);
}
...
static void handleCommand(long tag, char* rest) {
  ...
  if (strcmp(verb, "HOME") == 0) {
    ...
    if (!outerPresent && !innerPresent) {
      sendErr(tag, "ENOAXIS", "no stages detected");
      return;
    }
    if (strcmp(axis, "OUTER") != 0 && innerPresent)
      zaberHome(INNER_DEV, INNER_AX);
    if (strcmp(axis, "INNER") != 0 && outerPresent)
      zaberHome(OUTER_DEV, OUTER_AX);
    homed = false;
    beginMove(axis);
    state = S_HOMING;
    sendOk(tag, "ACCEPTED");
  ...
  if (strcmp(verb, "MOVE") == 0) {
    ...
    if (strcmp(axis, "OUTER") == 0) {
      ...
      zaberMoveDeg(OUTER_DEV, OUTER_AX, deg, OUTER_STEPS_PER_DEG);
      ...
      zaberMoveDeg(INNER_DEV, INNER_AX, deg, INNER_STEPS_PER_DEG);
  ...
  if (strcmp(verb, "MOVEB") == 0) {
    ...
    if (outerPresent) zaberMoveDeg(OUTER_DEV, OUTER_AX, o, OUTER_STEPS_PER_DEG);
    if (innerPresent) zaberMoveDeg(INNER_DEV, INNER_AX, i, INNER_STEPS_PER_DEG);
    beginMove("ALL");
    sendOk(tag, "ACCEPTED");
    return;
  }
...
void setup() {
  ...
  // Detect which stages are actually connected (works with 1 or 2).
  delay(200);
